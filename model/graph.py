...
# This is the code from incorporating graph into the model
...
class GraphLayer(nn.Module):
    def __init__(self, config, last=False):
        ...
        # bottleneck layer before output to avoid over-smoothing
    ...
    def forward(self, label_emb, extra_attn, self_attn_mask):
        """
        :param label_emb: [1, num_labels, hidden_size]
        """
        if GRAPH == 'GRAPHORMER':
            ...
            # label_emb.shape: [num_labels, hidden_size]
        # if self.last:
        ...
        label_emb = self.output_layer_norm(self.dropout(self.output_layer(label_emb)) + label_emb).unsqueeze(0)
        # label_emb.shape: [1, num_labels, hidden_size]
...
class GraphEncoder(nn.Module):
    def __init__(self, config, graph=False, layer=1, data_path=None, threshold=0.01, tau=1, label_dict=None):
        ...
        if graph:
            # Preloaded the graph structure in slot.pt
    ...
    def forward(self, embeddings):
        """
        embeddings: the mapping from token id to embedding
        """
        label_mask = self.label_name != self.tokenizer.pad_token_id # label_mask.shape: [label_num, max_len]
        # full name
        # check tokenizer max_len
        label_emb = embeddings(self.label_name) # label_emb.shape: [label_num, max_len, hidden_size]
        label_emb = (label_emb * label_mask.unsqueeze(-1)).sum(dim=1) / label_mask.sum(dim=1).unsqueeze(-1) # normalize
        label_emb = label_emb.unsqueeze(0)
        # label_emb.shape: [1, label_num, hidden_size]

        label_attn_mask = torch.ones(1, label_emb.size(1), device=label_emb.device) # label_attn_mask.shape: [1, label_num]

        extra_attn = None

        self_attn_mask = (label_attn_mask * 1.).t().mm(label_attn_mask * 1.).unsqueeze(0).unsqueeze(0) # self_attn_mask.shape: [1, 1, label_num, label_num]
